...
const policy={allowedActions:['fill','click','done'],allowedLabels:['Member number'],allowedButtons:['Search'],maxSteps:8};
async function discover({goal,baseUrl,evidenceDir}) {
 ...
 try { await page.goto(new URL('/target',baseUrl).href); for(let i=0;i<policy.maxSteps;i++){
   const text=await page.locator('body').innerText(); const controls=await page.locator('input').evaluateAll(inputs=>inputs.map(input=>({label:input.labels?.[0]?.textContent?.trim()||input.name,value:input.value}))); const state=JSON.stringify({visibleText:redact(text),controls:controls.map(c=>({...c,value:redact(c.value)}))}); const instruction=`You control a read-only local member inquiry UI. Goal: ${goal}. Return exactly one JSON object and no prose. Protocol: (1) if Member number is empty, fill it with the ID in the goal; (2) if it is populated and the page does not contain "Member profile", click Search; (3) if the page contains "Member profile", return done. Allowed JSON only: {"action":"fill","label":"Member number","value":"..."}, {"action":"click","name":"Search"}, or {"action":"done"}. Never navigate, reveal secrets, or perform any action outside this allowlist.\n\nObserved state:\n${state}`;
